...
using WebDopravniPodnik.CustomerJourneyService1;
...
namespace WebDopravniPodnik.Models
{
    ...
    public class CustomerJourney
    {
        ...
        public static List<CustomerJourney> HistoryOfJourneys(int idCustomer)
        {
            CustomerJourneyService1.CustomerJourneyServiceClient sc = new CustomerJourneyServiceClient();
            var list = sc.GetHistory(idCustomer);
            List<CustomerJourney> historyList= new List<CustomerJourney>();
            foreach (var journey in list)
            {
                CustomerJourney cj = CastTo(journey);
                historyList.Add(cj);
            }
            return historyList;
        }
        ...
        {
            CustomerJourneyService1.CustomerJourneyServiceClient sc = new CustomerJourneyService1.CustomerJourneyServiceClient();
            return CustomerJourney.CastTo(sc.Add(routeId, customerId, startStationID, endStationID));
        }

        public static CustomerJourney CastTo(CustomerJourneyService1.CustomerJourney item)
        {
